...
path='/home/vindhya/Documents/Siamese_Project/BreakHis/'+class_name
dest='/home/vindhya/Documents/Siamese_Project/BreakHis/'+class_name+"_patches_300/"
...
for file in files:
	...
	for i in range(0,row-window,np.int(window/slide)):
		for j in range(0,col-window,np.int(window/slide)):
			...
			ratio_white_pixel=float(pixel_and_count*100/(300*300))
			# print(ratio_white_pixel)
			if (ratio_white_pixel<40):
			 	cv2.imwrite(dest+img_name+"_"+str(i)+"_"+str(j)+".png",patch)
			# # prob0=patch[:,:,0].mean()
			# prob1=patch[:,:,1].mean()
			# prob2=patch[:,:,2].mean()
